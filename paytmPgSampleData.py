...
class SampleData:
    ...
    @staticmethod
    def get_money(amount_str):
        return Money(EnumCurrency.INR, amount_str)
    ...
    def get_user_info(member_uuid):
        ...
        if ' ' in name:
            ...
            lname = ''
        user_info = UserInfo()
        user_info.set_cust_id(member.uuid)
        user_info.set_address(member.address)
        user_info.set_email(member.email)
        user_info.set_first_name(fname)
        user_info.set_last_name(lname)
        user_info.set_mobile(member.phone)
        user_info.set_pincode("852201")
        return user_info
