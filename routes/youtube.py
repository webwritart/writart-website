    ...
    def get_project_dict_data(data, key, scene_no, shot_no):
        return next(
            (
                ...
                if str(scene.get("scene")) == str(scene_no)
                for shot in scene.get("shots", [])
                if str(shot.get("shot")) == str(shot_no)
            ),
            None
        )
        ...
        if youtube_img_creator in current_user.role or youtube_admin in current_user.role or youtube_seo_manager in current_user.role:
            ...
            if request.method == 'POST' and request.is_json:
                ...
                if data['type'] == 'get_img_upload_shot_data':
                    ...
                    scene_list = [a.scene for a in video_obj.storyboard_scenes]
                    ...
                    this_project_dict = video_obj.to_dict()
                    ...
                    if subtype == 'next':
                        if next_shot_number_single_shot_input(current_shot) in current_scene_shot_list:
                            ...
                            if str(int(current_scene) + 1) in scene_list:
                    ...
                    upload_media_next_shot_data_dict = {
                        ...
                        'scene': scene,
                        'shot': shot,
                        'shot_storyboard_img': get_project_dict_data(this_project_dict, "storyboard_img_path", scene, shot),
                        'shot_type': get_project_dict_data(this_project_dict, "shot_type", scene, shot),
                        'camera_direction': get_project_dict_data(this_project_dict, "frame_direction", scene, shot),
                        'direction': get_project_dict_data(this_project_dict, "creative_direction", scene, shot),
                        'narration_dialogue': get_project_dict_data(this_project_dict, "dialogue_narration", scene, shot),
...
def image_feedback():
    ...
    if not current_user.is_authenticated:
        ...
        if youtube_img_creator in current_user.role or youtube_admin in current_user.role:
            ...
            video = YoutubeVideo.query.filter_by(uuid=video_uuid).first()
            creatives_dict = video.to_dict()
            all_mates = [(a.uuid, a.name) for a in db.session.query(Member).all() if len([b for b in a.role if b.name == 'youtube_img_creator']) > 0]
        else:
            return render_template('admin_area.html')
        return render_template('image_feedback.html', logged_in=current_user.is_authenticated, admin=admin, youtube_admin=youtube_admin, 
                               youtube_img_creator=youtube_img_creator, mates=all_mates, creatives_dict=creatives_dict)
...
@youtube.route('/save-revision', methods=['POST'])
def save_revision():
    if request.method == 'POST' and request.form.get('type') == 'upload_revision':
        ...
        video_id = creative_obj.shot.scene.video.id
        channel_id = creative_obj.shot.scene.video.youtube_channel.id
        existing_uuid_list = [a.uuid for a in db.session.query(YoutubeVideoCreativeRevision)]
        uuid = create_uuid(existing_uuid_list, 9)
        ...
        if len(creative_obj.revisions) == 0:
            version = str(round(1.0+.1, 1))
        else:
            version = str(round(max([float(a.version) for a in creative_obj.revisions]) + .1, 1))

        entry = YoutubeVideoCreativeRevision(
            uuid=uuid,
            version=version,
            file_path=save_path[1:],
            text=revision_text,
        ...
        temp_assigned_uuid = creative_obj.assigned_to_uuid
        creative_obj.assigned_to_uuid = None
        creative_obj.last_assigned = temp_assigned_uuid
        db.session.commit()
...
def save_audio():
    if request.method == 'POST' and request.form.get('type') == 'save_creative_audio':
        ...
        video = creative_obj.shot.scene.video
        video_id = video.id
        channel_id = video.youtube_channel.id
        scene_shot = request.form.get('scene_shot')
        media_uuid =''
